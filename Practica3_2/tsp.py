...
class DiGraph:
    ...
    - self.forward es un diccionario que a cada vertice u le asocia
      una lista de pares (v,w)

    - self.backwards es un diccionario que a cada vertice v le asocia
      una lista de pares (u,w)

    forward es una lista de adyacencia (se vio en EDA)

    backwards es como la lista de adyacencia del grafo transpuesto
    (darle la vuelta a la orientacion de las aristas)
    '''
    
    def __init__(self):
        self.forward = {} # aristas que salen de cada vÃ©rtice
        # backwards solo se utiliza por Dijkstra cuando reverse=True
        # pero hay una cota que lo usa muchas veces
        self.backwards = {} # aristas que llegan a cada vÃ©rtice

    def nV(self):
        '''
        devuelve |V| nº vertices
        '''
    ...
    def nodes(self):
        ...
        de la clase sin acceder expli­citamente a self.forward
        '''
    ...
    def edges_from(self, u):
        ...
        sin acceder expli­citamente a un atributo
        '''
    ...
    def weight_edge(self, u, v):
        '''
        devuelve peso de la arista de u a v
    ...
    def Dijkstra(self, src, reverse=False):
        ...
        desde el vertice src
        Si ponemos reverse=True calcula las distancias
        desde cada vertice hasta src y no desde src al resto
        '''
        dist = { src:0 } # asocia una distancia a cada vertice
        pred = {} # para recuperar el camino de menor peso
        fixed = set() # cjt de vertices que ya han sido fijados
    ...
    def Dijkstra1dst(self, src, dst, avoid=()):
        '''
        Lanza el algoritmo de Dijkstra en el grafo
        desde el vertice src hasta el vertice dst
        y solo devuelve la distancia.
        Evita utilizar los vertices en avoid
        '''
    ...
    def StronglyCC(self):
        ...
        def SCCUtil(u): # funcion (closure) auxiliar, no es un metodo
            disc[u] = time[0]
            low[u] = time[0]
            time[0] += 1
            st.append(u)
...
def generate_random_digraph(nV, dimacsList=None, seed=None):
    '''
    recibe nº vertices
    tofile puede ser una lista Python (se la damos vacÃ­a),
    en cuyo caso escribe en ella el grafo en formato Dimacs:
    http://prolland.free.fr/works/research/dsat/dimacs.html

    seed permite inicializar la generacion de valores
    pseudo-aleatorios para facilitar la reproducibilidad
    de los experimentos
    '''
    rng = np.random.default_rng(seed)

    # calculamos unas coordenadas para cada vertice en un plano 2D
    xCoord = rng.uniform(0, 1000, size=nV)
    yCoord = rng.uniform(0, 1000, size=nV)

    # calculamos los grados de salida de todos vertices:
    gradomin = min(3,nV-1)
    gradomax = min(8,nV-1)
    ...
    for vertex in range(nV):
        ...
        for destination in destinations:
            ...
            # no es la distancia eucli­dea, aunque esti relacionada con
            # esta no es una metrica...
...
class BranchBoundImplicit:
    def solve(self):
        ...
        maxA       = 0 # tamaño maximo alzanzado por A
...
class BranchBoundExplicit:
    def solve(self):
        ...
        podas_opt  = 0 # nº podas por cota optimista
        maxA       = 0 # tamaño maximo alzanzado por A
        cost_expl  = 0 # coste poda expli­cita
        podas_expl = 0 # podas dentro de A con la poda expli­cita
        # bucle principal ramificacion y poda (PODA IMPLICITA)
        while len(A)>0:
            iterations += 1
            lenA = len(A)
...
class TSP:
    ...
    def is_factible(self, s):
        ...
        el ultimo vertice s[-1] hasta el primero s[0]
        '''
...
class TSP_Cota1(TSP):
    '''
    Version con cota trivial. Solamente se suma la parte conocida.
...
class TSP_Cota4(TSP):
    '''
    Las aristas de minimo coste que parten de cada vertice
    no visitado (mas el ultimo visitado). 
    Es facil calcularla de manera incremental.
    '''
...
class TSP_Cota5(TSP):
    '''
    Las aristas de mÃ­nimo coste que parten de cada vÃ©rtice
    no visitado (mÃ¡s el Ãºltimo visitado)
    y llegan a vÃ©rtices no visitados o al vÃ©rtice origen.
    No es incremental.
    '''
...
class TSP_Cota6(TSP):
    ...
    TambiÃ©n se puede calcular de modo incremental tras
    aplicar el algoritmo de Dijkstra desde el vÃ©rtice
    inicial del camino sobre el grafo traspuesto
    (usando las aristas en direcciÃ³n contraria) para asÃ­
...
class TSP_Cota7(TSP):
    ...
    el primero y el Ãºltimo de la soluciÃ³n parcial).
...
# algunas cotas son tan lentas que mejor fijarles una talla maxima
tallaMax = {'Cota1I':15,
            'Cota1E':15,
            'Cota4I':20,
            'Cota4E':20,
...
def experimento():
    ...
    - Para cada talla generar entre 5 y 20 instancias (segÃºn lo lento
      que funcione)
    - IMPORTANTE probar diversos algoritmos con LAS MISMAS instancias
    - Descartar las que no den soluciÃ³n (las que sacan None,float('inf'))
    - Mostrar valores medios de todas las instancias de cada talla.
    - Sacar los datos de manera que sea fÃ¡cil realizar una interpretacion
    '''
